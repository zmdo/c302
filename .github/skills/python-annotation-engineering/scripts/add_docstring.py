...
def run_gen_index(file_path: Path) -> None:
    ...
    从本脚本所在目录或项目中查找 gen_index.py，兼容不同的项目布局。
    查找顺序：同目录 → 向上查找 python-annotation-engineering → python-code-style。
    若找不到则静默跳过（不影响主流程）。
    ...
    # 优先查找同目录下的 gen_index.py（skill 内置场景）
    sibling = Path(__file__).parent / "gen_index.py"
    if sibling.exists():
        gen_index = sibling
    else:
        # 向上查找项目中的 gen_index.py（兼容多种 skill 布局）
        for parent in Path(__file__).parents:
            for skill_name in ("python-annotation-engineering", "python-code-style"):
                candidate = (
                    parent / ".github" / "skills" / skill_name / "scripts" / "gen_index.py"
                )
                if candidate.exists():
                    gen_index = candidate
                    break
