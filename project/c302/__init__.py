# =============================================================================
# 功能描述：
#   c302 框架的核心网络生成模块。将连接组数据（connectome）与参数化细胞/突触
#   模型组装为完整的 NeuroML2 网络，并输出 .net.nml 和 LEMS 仿真文件。
#   包含 CLI 入口、数据读取器加载、种群创建、连接遍历、刺激注入等功能。
#
# 类与方法索引：
#   print_                               (L138)  — 带 ``c302`` 前缀的调试输出，用于区分框架自身的日志信息
#   load_data_reader                     (L149)  — 动态导入并返回指定名称的数据读取器模块
#   get_str_from_exponential             (L162)  — 将浮点数格式化为 15 位小数的字符串表示
#   get_muscle_position                  (L173)  — 根据肌肉名称计算其在虫体中的三维坐标
#   is_muscle                            (L209)  — 判断细胞名称是否为肌肉（匹配 ``M[VD][LR]<digits>`` 模式）
#   process_args                         (L218)  — 解析命令行参数
#   get_next_stim_id                     (L383)  — 为指定细胞生成下一个不重复的刺激 ID
#   get_cell_position                    (L401)  — 从 NeuroML 多室形态文件中读取细胞 soma 位置
#   append_input_to_nml_input_list       (L421)  — 将刺激输入追加到 NeuroML 网络的 ``InputList`` 中
#   add_new_sinusoidal_input             (L443)  — 为指定细胞创建正弦波刺激输入
#   add_new_input                        (L487)  — 为指定细胞创建脉冲刺激输入（``PulseGenerator``）
#   get_muscle_names                     (L504)  — 生成全部 96 条体壁肌肉的名称列表
#   merge_with_template                  (L525)  — 使用 Airspeed 模板引擎将变量字典与 LEMS 模板文件合并
#   write_to_file                        (L537)  — 将生成的 NeuroML 网络和 LEMS 仿真文件写入磁盘
#   get_projection_id                    (L592)  — 根据突触前/后细胞和突触类型生成标准 Projection ID
#   get_random_colour_hex                (L611)  — 生成随机十六进制颜色字符串（``#RRGGBB``），用于绘图颜色分配
#   get_file_name_relative_to_c302       (L627)  — 返回相对于 ``C302_HOME`` 环境变量的文件路径
#   get_cell_names_and_connection        (L637)  — 读取连接组数据，返回所有细胞名称和突触连接列表
#   get_cell_muscle_names_and_connection (L660)  — 读取神经元-肌肉连接数据，返回运动神经元、已知肌肉列表和肌肉连接
#   is_cond_based_cell                   (L693)  — 判断参数层级是否为导电模型（Level C 或 D 系列）
#   get_cell_id_string                   (L702)  — 构建 NeuroML 中引用细胞实例的路径字符串
#   regex_match                          (L728)  — 当 pattern 为正则表达式时执行匹配
#   is_regex_string                      (L738)  — 判断字符串是否为正则表达式格式（同时含 ``^`` 和 ``$``）
#   elem_in_coll_matches_conn            (L747)  — 检查集合中是否有正则元素匹配给定的连接字符串
#   _get_cell_info                       (L763)  — 从 owmeta Bundle 或本地缓存获取细胞的详细注释信息
#   set_param                            (L861)  — 设置或新增生物参数值
#   mirror_param                         (L881)  — 为双向缝隙连接参数设置镜像值（A-B 和 B-A 使用相同参数）
#   generate                             (L906)  — c302 网络生成主入口，将连接组数据转化为完整的 NeuroML2 网络
#   parse_list_arg                       (L2065) — 解析 CLI 列表参数字符串为 Python 列表
#   parse_dict_arg                       (L2087) — 解析 CLI 字典参数字符串为 Python 字典
#   main                                 (L2108) — c302 CLI 主入口，解析命令行参数并调用 ``generate()``
#
# 更新日志：
