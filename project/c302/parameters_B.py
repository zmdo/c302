...
#
# 类与方法索引：
#   ParameterisedModel                   (L54)   — ParameterisedModel 类
#     __init__                           (L55)   — 初始化 Level B 参数模型，继承 Level A 并添加真实缝隙连接和 activity 变量
#     set_default_bioparameters          (L68)   — 设置 Level B 的默认生物参数，在 Level A 基础上新增 tau1 和真实电突触参数
#     create_generic_muscle_cell         (L108)  — 创建带 activity 变量的通用肌肉细胞（``IafActivityCell``）
#     create_generic_neuron_cell         (L124)  — 创建带 activity 变量的通用神经元细胞（``IafActivityCell``）
#     create_neuron_to_neuron_syn        (L136)  — 创建神经元间化学突触（``ExpTwoSynapse``）和真实缝隙连接（``GapJunction``）
#     create_neuron_to_muscle_syn        (L163)  — 创建神经元到肌肉的化学突触（``ExpTwoSynapse``）和缝隙连接（``GapJunction``）
#     get_elec_syn                       (L186)  — 根据连接类型获取真实缝隙连接（``GapJunction``）对象
#   IafActivityCell                      (L231)  — IafActivityCell 类
#     __init__                           (L232)  — 初始化带 activity 变量的积分放电细胞，存储所有膜特性参数
#     export                             (L254)  — 将细胞定义以 NeuroML XML 格式写入输出流
#
# 更新日志：
